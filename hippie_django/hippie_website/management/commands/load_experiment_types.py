...
def check_if_name_exists(name):
    if ExperimentType.objects.filter(name=name).exists():
        ...
        raise CommandError(
            f"Name {name!r} already exists with PSI-MI code {obj.psi_mi_code!r}."
        )
    return False
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            default=str(
                ...
                / "data"
                / "techniques_scoring_04-05-26.csv"
            ),
    ...
    def handle(self, **options) -> None:
        ...
        with path.open(newline="", encoding="utf-8") as fh:
            ...
            for line_number, row in enumerate(reader, start=2):
                ...
                try:
                    ...
                    self.stderr.write(
                        f"Line {name}:{psi_mi_code}: invalid score {new_score!r}, skipping"
                    )
                    skipped += 1
                    continue

                if psi_mi_code == "":
                    obj, was_created = ExperimentType.objects.get_or_create(name=name)
                else:
                    ...
                    if not was_created and obj.name != name:
                        conflict_msg = f"line={line_number}\tpsi_mi={psi_mi_code}\texisting_name={obj.name}\tincoming_name={name}"
                        if obj.name in rename_dict:
                            new_name = rename_dict[obj.name]
                            conflict_log_lines.append(
                                f"{conflict_msg}\taction=rename_cache\tresult={new_name}"
                            )
                        else:
                            if on_conflict:
                                confirm = on_conflict
                            ...
                            if confirm.lower() in {"di", "discard"}:
                                ...
                                new_name = name
                                conflict_log_lines.append(
                                    f"{conflict_msg}\taction=change"
                                )
                            else:
                                raise CommandError(
                                    "Invalid input, expected 'k', 'ch' or 'di'."
                                )
                        obj.name = new_name
        ...
        if on_conflict and conflict_log_lines:
            ...
            conflict_log_path = (
                log_dir / f"load_experiment_types_conflicts_{datetime.now().date()}.log"
            )
